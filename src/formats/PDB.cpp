...
/// message
static void check_values_size(const Vector3D &values, unsigned width,
...
size_t PDBFormat::nsteps() { return steps_positions_.size(); }
...
void PDBFormat::read_CRYST1(Frame &frame, const std::string &line) {
  ...
  if (line.length() >= 55) {
    ...
    if (space_group != "P 1" && space_group != "P1") {
      warning("Space group which is not P1 ({}) ignored in '{}'", space_group,
              file_->filename());
    }
  }
}
...
void PDBFormat::read_ATOM(Frame &frame, const std::string &line,
  ...
  struct Property tmp = {is_hetatm};
  atom.set("is_hetatm", tmp);
...
void PDBFormat::read_CONECT(Frame &frame, const std::string &line) {
  ...
  auto read_index = [&line](size_t initial) -> size_t {
    try {
      ...
      throw FormatError("Could not read atomic number at index " +
                        std::to_string(initial) + " in : '" + trim(line) + "'");
    }
  };
...
void PDBFormat::write(const Frame &frame) {
  ...
  for (size_t i = 0; i < frame.natoms(); i++) {
    ...
    if (residue) {
      ...
      if (resname.length() > 3) {
        warning("Residue '{}' has a name too long for PDB format, it will be "
                "truncated.",
                resname);
    ...
    fmt::print(*file_, "HETATM{: >5} {: >4s} {:3} X{: >4s}    "
                       "{:8.3f}{:8.3f}{:8.3f}{:6.2f}{:6.2f}          {: >2s}\n",
               to_pdb_index(i), name, resname, resid, pos[0], pos[1], pos[2],
               1.0, 0.0, type);
  }
...
void check_values_size(const Vector3D &values, unsigned width,
                       const std::string &context) {
  ...
  if (values[0] > max_pos || values[1] > max_pos || values[2] > max_pos ||
    ...
    throw FormatError("Value in " + context +
                      " is too big for representation in PDB format");
  }
}
