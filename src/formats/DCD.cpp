...
template<> FormatInfo chemfiles::format_information<DCDFormat>() {
   return FormatInfo("DCD").with_extension(".dcd").description(
      "DCD () single precision binary FORTRAN format"
   );
}
...
void set_cell(matrix box, Frame& frame) {
   ...
   frame.set_cell({ a.norm(), b.norm(), c.norm(), alpha, beta, gamma });
}
