...
/// two vectors are exactly the same.
...
inline double norm2(const Vector3D &rhs) { return dot(rhs, rhs); }

/// Compute the euclidean norm of a vector.
inline double norm(const Vector3D &rhs) { return std::sqrt(norm2(rhs)); }
...
// is compatible with the C type `(*double)[3] == chfl_vector3d`.
...
inline bool operator==(const chemfiles::Matrix3D &lhs,
                       const chemfiles::Matrix3D &rhs) {
  return lhs[0][0] == rhs[0][0] && lhs[1][0] == rhs[1][0] &&
         lhs[2][0] == rhs[2][0] && lhs[0][1] == rhs[0][1] &&
         lhs[1][1] == rhs[1][1] && lhs[2][1] == rhs[2][1] &&
...
inline Matrix3D matrix3d() { return matrix3d(0, 0, 0); }
...
struct Property {
  bool as_bool() const {
    if (kind_ == BOOL) {
      ...
      throw Error("Property is not of type bool");
    }
  }
  ...
  const char *as_char() const {
    if (kind_ == CHAR) {
      ...
      throw Error("Property is not of type char");
    }
  }
  ...
  double as_double() const {
    if (kind_ == DOUBLE) {
      ...
      throw Error("Property is not of type double");
    }
  }
  ...
  Vector3D as_vector3d() const {
    if (kind_ == VECTOR3D) {
      ...
      throw Error("Property is not of type Vector3D");
    }
  }
