...
namespace chemfiles {
	...
	public:
		...
		std::vector<int>* searchBonds(std::vector<Vector3D> pos, int natoms, float cutoff, int maxBonds);

		int generateBonds(std::vector<int>* result, std::vector<Vector3D>* positions, int totalCells, int** cells,
